package com.example.newsroom.app.di

import com.example.newsroom.app.data.remote.NewsApi
import com.example.newsroom.app.data.remote.NewsApi.Companion.BASE_URL
import com.example.newsroom.app.data.repositoryImpl.NewsRepositoryImpl
import com.example.newsroom.app.domain.repository.NewsRepository
import dagger.Module
import dagger.Provides
import dagger.hilt.InstallIn
import dagger.hilt.components.SingletonComponent
import retrofit2.Retrofit
import retrofit2.converter.gson.GsonConverterFactory
import javax.inject.Singleton


@Module // to get instance from retrofit
@InstallIn(SingletonComponent::class)
object AppModule {

    @Provides
    @Singleton //create single object for whole app
//    مش كل شويه تعملي اوبجكت مختلف
    fun provideNewsApi(): NewsApi{
        val retrofit= Retrofit.Builder()
            .baseUrl(BASE_URL)
            .addConverterFactory(GsonConverterFactory.create())
            .build()
        return retrofit.create(NewsApi::class.java)
    }
    @Provides
    @Singleton
    fun provideNewsRepository(newsApi: NewsApi): NewsRepository{
        return NewsRepositoryImpl(newsApi = newsApi)
    }

}